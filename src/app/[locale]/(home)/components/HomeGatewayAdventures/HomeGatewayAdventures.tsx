...
export const HomeGatewayAdventures = () => {
  ...
  const mobileTours = useMemo(() => tours.slice(0, 4), [tours]);
  ...
  const renderTourCard = useCallback(
    (tour: (typeof tours)[number], key: string, isExpanded = false) => (
      <div
        key={key}
        className={`${styles.home_gateway__slide} ${
          isExpanded ? styles.home_gateway__slideActive : ""
        }`}
      >
        <article className={styles.home_gateway__card}>
          <div className={styles.home_gateway__cardMedia}>
            ...
            <div className={styles.home_gateway__cardTop}>
              <div className={styles.home_gateway__cardMeta}>
                <span className={styles.home_gateway__area}>{tour.area}</span>
          ...
          <div className={styles.home_gateway__cardBottom}>
            <p className={styles.home_gateway__price}>{tour.price}</p>
            ...
            <Button
              ...
              url={`/tours/${tour.slug}`}
            >
              <span>{t("addToCart", { fallback: "Add to cart" })}</span>
              <span
          ...
          >
            ...
            >
              {slides.map((tour, index) =>
                renderTourCard(tour, `${tour.key}-${index}`, index === displayIndex),
              )}
            </div>
          </div>

          <div className={styles.home_gateway__mobileList}>
            {mobileTours.map((tour) => renderTourCard(tour, tour.key, true))}
          </div>
